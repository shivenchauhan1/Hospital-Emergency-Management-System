...
export default function EmergencyPage({ emergencyCases, doctors, onUpdateEmergencyCase }) {
  ...
  const [assignedDoctor, setAssignedDoctor] = useState('Dr. Gregory House');
  const [updatedStatus, setUpdatedStatus] = useState('In Resuscitation');

  const handleOpenAssignModal = (caseItem) => {
    setSelectedCase(caseItem);
    setAssignedDoctor(caseItem.doctor || doctors[0]?.name || 'Dr. Gregory House');
    ...
    switch (priority) {
      case 'Red':
        return {
          ...
          border: 'border-l-4 border-l-red-600 border-[#DC2626]/30',
          label: 'Critical (Red)'
        };
      case 'Orange':
        return {
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 bg-gradient-to-r from-red-950 via-slate-900 to-slate-950 p-6 sm:p-8 rounded-3xl border border-red-900/50 shadow-2xl text-white">
        <div>
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full text-xs font-black bg-red-600/30 text-red-300 border border-red-500/40 mb-2">
            ...
            <span>Emergency Resuscitation & Triage Unit</span>
          </div>
          ...
          <p className="text-slate-300 text-xs sm:text-sm mt-1">
            Real-time triage monitoring, emergency priority severity management, and rapid doctor dispatch.
          </p>
        </div>

        {/* Priority Color Legend */}
        <div className="flex flex-wrap items-center gap-2 text-xs font-bold font-mono">
          <span className="px-2.5 py-1 rounded-lg bg-red-600 text-white shadow-sm">Red = Critical</span>
          <span className="px-2.5 py-1 rounded-lg bg-orange-500 text-white shadow-sm">Orange = High</span>
          ...
          return (
            ...
            >
              ...
              <div className="space-y-2 text-xs text-slate-600 font-medium">
                ...
                <div className="flex items-center justify-between border-b border-slate-100 pb-1.5">
                  <span className="text-slate-400 flex items-center gap-1">
                    <Stethoscope className="w-3.5 h-3.5 text-teal-600" /> Assigned Doctor:
                  </span>
                ...
                <div className="flex items-center justify-between border-b border-slate-100 pb-1.5">
                  <span className="text-slate-400 flex items-center gap-1">
                    <Activity className="w-3.5 h-3.5 text-purple-600" /> Current Status:
                  </span>
                  <span className="font-bold text-teal-700">{c.status}</span>
                </div>
              ...
              <div className="pt-2 grid grid-cols-3 gap-2">
                <button
                  ...
                  className="py-2 px-2 rounded-xl text-[11px] font-extrabold bg-teal-50 text-[#0F766E] border border-teal-200 hover:bg-teal-100 transition-colors text-center"
                >
                  Assign Doctor
                </button>
                ...
                <button
                  ...
                  className="py-2 px-2 rounded-xl text-[11px] font-extrabold bg-[#0F766E] text-white hover:bg-teal-800 transition-colors text-center"
                >
                  View Case
                </button>
              </div>
            ...
            <form onSubmit={handleSaveDoctor} className="space-y-4">
              <div>
                ...
                <select
                  ...
                  className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3.5 py-2 text-xs font-extrabold text-slate-900 focus:outline-none focus:border-[#0F766E]"
                >
                  {doctors.map(d => (
      ...
      {isStatusModalOpen && selectedCase && (
        <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl max-w-md w-full p-6 shadow-2xl space-y-5 relative">
            ...
            <div>
              <h3 className="text-lg font-extrabold text-slate-900">Update Emergency Status</h3>
              <p className="text-xs text-slate-500 mt-0.5">Emergency Case: {selectedCase.id} ({selectedCase.patientName})</p>
            </div>
            ...
            <form onSubmit={handleSaveStatus} className="space-y-4">
              <div>
                ...
                <select
                  ...
                  className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3.5 py-2 text-xs font-extrabold text-slate-900 focus:outline-none focus:border-[#0F766E]"
                >
                  <option>In Resuscitation</option>
                  <option>Administering Medication</option>
                  <option>In Pre-Op Surgery</option>
                  <option>Stabilized & Transferred to ICU</option>
                  <option>Under Observation in ER</option>
