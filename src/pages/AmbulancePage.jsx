...
export default function AmbulancePage({ ambulances, onUpdateAmbulanceStatus }) {
  ...
  const handleOpenDispatchModal = (amb) => {
    ...
    setDispatchLocation('Tribune Chowk, Sector 29, Chandigarh');
  ...
  const handleDispatchSubmit = (e) => {
    ...
    onUpdateAmbulanceStatus(selectedAmbulance.id, 'On Route', dispatchLocation);
      ...
      <div className="w-full flex flex-wrap items-center justify-between gap-4 bg-white p-6 sm:p-8 rounded-3xl border border-slate-200 shadow-sm">
        <div>
          <h1 className="text-2xl sm:text-3xl font-black text-slate-900 flex items-center gap-3">
            <Truck className="w-8 h-8 text-[#00695C]" />
            108 Ambulance Dispatch & Paramedic Fleet
          </h1>
          <p className="text-slate-500 text-xs sm:text-sm mt-1">
            Real-time 108 emergency ambulance tracking, paramedic team dispatching across Chandigarh, Mohali & Panchkula.
          </p>
        </div>
        <div className="flex items-center gap-3 text-xs font-mono font-bold">
          <span className="px-3 py-1.5 rounded-xl bg-emerald-50 text-emerald-800 border border-emerald-200">
            {ambulances.filter(a => a.status === 'Available').length} Fleet Ambulances Available
          </span>
        </div>
      </div>
        ...
        {ambulances.map((amb) => (
          ...
          >
            <div className="space-y-3">
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <div className="w-10 h-10 rounded-xl bg-teal-50 text-[#00695C] flex items-center justify-center font-bold">
                    <Truck className="w-5 h-5" />
                  </div>
                  <div>
                    <h3 className="text-base font-black text-slate-900 font-mono">{amb.number}</h3>
                ...
                <span className={`px-2.5 py-1 rounded-full text-xs font-extrabold ${
                  ...
                  amb.status === 'On Route' ? 'bg-red-100 text-red-800 border border-red-200 animate-pulse' :
              ...
              <div className="p-3 rounded-2xl bg-slate-50 border border-slate-200 space-y-2 text-xs">
                ...
                <div className="flex items-center justify-between">
                  <span className="text-slate-400 font-bold text-[10px] uppercase">GPS Station Location:</span>
                  <span className="font-mono font-bold text-teal-900 truncate max-w-[170px]">{amb.location}</span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-slate-400 font-bold text-[10px] uppercase">ETA to Sanjeevani:</span>
            ...
            <div className="pt-2 grid grid-cols-2 gap-2">
              <button
                ...
                className={`py-2 px-3 rounded-xl text-xs font-extrabold transition-colors flex items-center justify-center gap-1.5 ${
                  amb.status === 'Maintenance'
                    ...
                    : 'bg-[#00695C] text-white hover:bg-teal-800 shadow-sm'
                }`}
              >
                <Send className="w-3.5 h-3.5" /> Dispatch 108
              </button>
              ...
              >
                <Navigation className="w-3.5 h-3.5 text-[#00695C]" /> Live GPS
              </button>
            </div>
          </div>
        ))}
      ...
      {isDispatchModalOpen && selectedAmbulance && (
        <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl max-w-md w-full p-6 shadow-2xl space-y-5 relative">
            ...
            <div>
              <h3 className="text-lg font-extrabold text-slate-900">Dispatch 108 Ambulance ({selectedAmbulance.number})</h3>
              <p className="text-xs text-slate-500 mt-0.5">Paramedic: {selectedAmbulance.driver} ({selectedAmbulance.type})</p>
            </div>

            <form onSubmit={handleDispatchSubmit} className="space-y-4">
              <div>
                <label className="text-xs font-bold text-slate-700 block mb-1">Emergency Pick-up Location Address</label>
                <input
                  type="text"
                  required
                  placeholder="e.g. Tribune Chowk, Sector 29, Chandigarh"
                  value={dispatchLocation}
                  onChange={(e) => setDispatchLocation(e.target.value)}
                  className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3.5 py-2 text-xs font-semibold text-slate-900 focus:outline-none focus:border-[#00695C]"
                />
              </div>
              ...
              <div className="flex items-center justify-end gap-3 pt-2">
                ...
                <button
                  type="submit"
                  className="px-5 py-2 rounded-xl text-xs font-bold text-white bg-[#D32F2F] hover:bg-red-700 shadow-md shadow-red-600/20"
                >
                  Authorize 108 Dispatch
                </button>
              </div>
            </form>
          </div>
        ...
        <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl max-w-lg w-full p-6 sm:p-8 shadow-2xl space-y-5 relative">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-xl font-extrabold text-slate-900">108 Live GPS Fleet Radar</h3>
                <p className="text-xs font-mono font-bold text-teal-800">Vehicle Reg: {selectedAmbulance.number}</p>
              </div>
            </div>
            ...
            <div className="h-48 bg-slate-900 rounded-2xl border border-slate-800 p-4 relative overflow-hidden flex flex-col justify-between text-white">
              ...
              <div className="flex items-center justify-between text-xs relative z-10 font-mono">
                <span className="text-teal-400 font-bold">● GPS Live Telematics</span>
                <span className="text-slate-400">Signal: 99% (4G GPS)</span>
              </div>
              ...
              <div className="text-center space-y-1 relative z-10">
                ...
                <div className="text-[11px] text-teal-300">ETA to Sanjeevani ER: {selectedAmbulance.ETA}</div>
              </div>
